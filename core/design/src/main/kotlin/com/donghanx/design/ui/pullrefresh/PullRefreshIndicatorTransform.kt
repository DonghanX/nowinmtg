...
fun Modifier.pullRefreshIndicatorTransform(state: PullRefreshState, scale: Boolean = false) =
    inspectable(
        ...
        Modifier
            // Essentially we only want to clip the at the top, so the indicator will not appear
            // when
            // the position is 0. It is preferable to clip the indicator as opposed to the layout
            // that
            // contains the indicator, as this would also end up clipping shadows drawn by items in
            // a
            // list for example - so we leave the clipping to the scrolling container. We use
            // MAX_VALUE
            // for the other dimensions to allow for more room for elevation / arbitrary indicators
            // - we
